var toopherWeb = (function($){
  ...
  var getTerminalId = function(iframe, callback) {
    ...
    if (terminalName) {
      ...
      var tDiv = $('<div>Please enter a name for this terminal:<input type="text" id="toopher-terminal-name-input"></input><input id="toopher-terminal-name-button" type="button" value="OK"></input></div>');
      tDiv.children('#toopher-terminal-name-button').click(function(){
        terminalName = tDiv.children('#toopher-terminal-name-input').val();
  ...
  var handleMessage = function(e){
    ...
    $.post(iframe.attr('toopher-postback'), e.data);
  }
  ...
  var init = function(target){
    getTerminalId(target, function(terminalName){
      toopher_url = target.attr('toopher-req') + '&terminal_name=' + encodeURIComponent(terminalName);
